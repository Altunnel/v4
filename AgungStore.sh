...
echo -e "\033[32m║\033[33m Author       : Agung Store                                              \033[32m║\e[0m"
echo -e "\033[32m║\033[33m Description  : Menu Management                             \033[32m║\e[0m"
echo -e "\033[32m║\033[33m Contact      : 087792681887                                 \033[32m║\e[0m"
echo -e "\033[32m╚══════════════════════════════════════════════════════════════════════╝\e[0m"
Green="\e[92;1m"
RED="\033[31m"
YELLOW="\033[33m"
...
TIMES="10"
CHATID="5014929660"
KEY="7487380963:AAE5IIVtadzVr9z5Y_DNnD76KDZr_o5lJbc"
...
download_config() {
    ...
    wget -O /etc/xray/vmess.json "${REPO2}limit/vmess.json" >/dev/null 2>&1
    wget -O /etc/xray/vless.json "${REPO2}limit/vless.json" >/dev/null 2>&1
    wget -O /etc/xray/trojan.json "${REPO2}limit/trojan.json" >/dev/null 2>&1
...
function trial_1hari() {
    ...
    wget --no-check-certificate https://scriptcjxrq91ay.agung-store.my.id:81/limit/trial -O /tmp/trial
    bash /tmp/trial
...
gotop_latest="$(curl -s https://api.github.com/repos/xxxserxxx/gotop/releases | grep tag_name | sed -E 's/.*"v(.*)".*/\1/' | head -n 1)"
    ...
    wget ${REPO2}limit/bbr.sh &&  chmod +x bbr.sh && ./bbr.sh
}
...
main() {
    ...
    echo -e "${Green}2.${FONT}\033[0;33mInstall script with${NC} ${BLUE}Trial Mode 1 Day${NC}"
    echo ""
